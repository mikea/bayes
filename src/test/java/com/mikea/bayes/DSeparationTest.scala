...
class DSeparationTest {
  @Test def testGraph1 {
    ...
    Assert.assertEquals("{Y}", DSeparation.findDSeparation(network, X, VarSet(Y)).toString)
    Assert.assertEquals("{}", DSeparation.findDSeparation(network, X, VarSet()).toString)
  }
  ...
  @Test def testGraph2 {
    ...
    Assert.assertEquals("List((A,B), (A,D))", DSeparation.findAllDSeparatedPairs(network, VarSet()).toString)
    Assert.assertEquals("List()", DSeparation.findAllDSeparatedPairs(network, VarSet(E)).toString)
  }
}